...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="max-w-2xl">
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                <form action="{{ route('admin.blog-categories.update', $blogCategory) }}" method="POST" data-controller="slug-generator">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input type="text" name="name" id="name" class="input input-bordered @error('name') input-error @enderror" 
                               value="{{ old('name', $blogCategory->name) }}" required data-slug-generator-target="source">
                        @error('name')
                            <label class="label">
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input type="text" name="slug" id="slug" class="input input-bordered @error('slug') input-error @enderror" 
                               value="{{ old('slug', $blogCategory->slug) }}" data-slug-generator-target="slug">
                        @error('slug')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                            </label>
                    ...
                    <div class="form-control mb-6">
                        ...
                        <textarea name="description" id="description" rows="3" 
                                  class="textarea textarea-bordered @error('description') textarea-error @enderror">{{ old('description', $blogCategory->description) }}</textarea>
                        @error('description')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                            </label>
