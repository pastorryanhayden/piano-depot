...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form action="{{ route('admin.blog-posts.update', $blogPost) }}" method="POST" enctype="multipart/form-data" data-controller="blog-form">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="form-control">
                            ...
                            <input type="text" name="title" id="title" class="input input-bordered @error('title') input-error @enderror" 
                                   value="{{ old('title', $blogPost->title) }}" required data-blog-form-target="title">
                            @error('title')
                                <label class="label">
                        ...
                        <div class="form-control">
                            ...
                            <input type="text" name="slug" id="slug" class="input input-bordered @error('slug') input-error @enderror" 
                                   value="{{ old('slug', $blogPost->slug) }}" data-blog-form-target="slug">
                            @error('slug')
                                <label class="label">
                                    <span class="label-text-alt text-error">{{ $message }}</span>
                                </label>
                        ...
                        <div class="form-control">
                            ...
                            <textarea name="excerpt" id="excerpt" rows="3" class="textarea textarea-bordered @error('excerpt') textarea-error @enderror">{{ old('excerpt', $blogPost->excerpt) }}</textarea>
                            @error('excerpt')
                                <label class="label">
                                    <span class="label-text-alt text-error">{{ $message }}</span>
                                </label>
                        ...
                        <div class="form-control">
                            ...
                            <input type="hidden" name="content" id="content" value="{{ old('content', $blogPost->content) }}" data-blog-form-target="content">
                            @error('content')
                                <label class="label">
                                    <span class="label-text-alt text-error">{{ $message }}</span>
                                </label>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-control">
                            <label class="label cursor-pointer">
                                ...
                                <input type="checkbox" name="is_published" class="checkbox" value="1" 
                                       {{ old('is_published', $blogPost->is_published) ? 'checked' : '' }} data-blog-form-target="publishNow">
                            </label>
                        </div>
                        ...
                        <div class="form-control">
                            ...
                            <input type="datetime-local" name="published_at" id="published_at" class="input input-bordered input-sm" 
                                   value="{{ old('published_at', $blogPost->published_at?->format('Y-m-d\TH:i')) }}" data-blog-form-target="publishDate">
                            @error('published_at')
                                <label class="label">
                                    <span class="label-text-alt text-error">{{ $message }}</span>
                                </label>
                            @enderror
                            <label class="label">
                                <span class="label-text-alt">Schedule for future publishing</span>
                            </label>
                        </div>
                ...
                <div class="card bg-base-100 shadow-xl">
                    <div class="card-body">
                        ...
                        @if($blogPost->featured_image)
                            <div class="mb-4">
                                <img src="{{ Storage::url($blogPost->featured_image) }}" alt="Current featured image" class="w-full rounded-lg">
                        ...
                        <div class="form-control">
                            <label for="featured_image" class="label">
                                <span class="label-text">{{ $blogPost->featured_image ? 'Replace Image' : 'Upload Image' }}</span>
                            </label>
